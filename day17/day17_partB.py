...
def determine_next_cube_state(x,y,z,w):
            ...
            for c in range(z-1, z+2):
                for d in range(w-1, w+2):
                    if (a,b,c,d) == (x,y,z,w):
                        ...
                        continue
                    if (a,b,c,d) in state_cycleNum[i1]:
    ...
    if (x,y,z,w) in state_cycleNum[i1]:  # If cube active
        if number_active_neighbors in [2,3]: # If 2 or 3 neighbors
            state_cycleNum[i1+1].add((x,y,z,w))
...
with open(input_filename) as f:
    ...
    for line_num, in_string in enumerate(f):
        ...
        initial_state.extend([ (a + number_of_cycles , number_of_cycles + len(in_string) - line_num - 1, number_of_cycles, number_of_cycles) for a in range(len(in_string)) if in_string[a] == '#' ])
...
for i1 in range(number_of_cycles):
    ...
    for (x,y,z,w) in state_cycleNum[i1]:
        cubes_to_evaluate.update([(a,b,c,d) for a in range(x-1, x+2) for b in range(y-1, y+2) for c in range(z-1, z+2) for d in range(w-1, w+2)])

    for (x,y,z,w) in cubes_to_evaluate:
        determine_next_cube_state(x,y,z,w)

    print(f'Cycle #{i1+1}: {len(state_cycleNum[i1+1])}')

print(f'\nThe solution to part B is {len(state_cycleNum[i1+1])}\n')
